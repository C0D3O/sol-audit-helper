...
const generateSlocReport = async (cwd: string, scopeNames: string[], newPath: string) => {
	// CREATE A SLOC REPORT
	const winCwd = cwd.replaceAll('/', '\\');

	writeFileSync(process.platform === 'win32' ? `${winCwd}\\sLoc.html` : `${cwd}\\sLoc.html`, htmlTemplate);
	let id = 'a';
